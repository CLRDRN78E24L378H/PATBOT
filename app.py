...
# FUNZIONE SPECIALE: Carica i PDF su Google UNA VOLTA SOLA e li tiene in memoria per non consumare il limite
...
# Avviamo lo studio dei documenti in un blocco protetto per evitare crash da limite superato
...
if "chat_engine" not in st.session_state and documenti_caricati:
    ...
    2. ANCORAGGIO TEMPORALE E REGIME TRANSITORIO: La data di oggi è {data_odierna}. Considera ogni domanda come posta in questo esatto momento. Se l'utente usa un tempo futuro o un condizionale (es. "cosa succede se al terzo anno non vive con la madre?"), devi interpretarla come un'ipotesi futura rispetto a oggi, applicando la normativa attualmente in vigore o il regime transitorio pertinente alla data odierna.
    
    3. LIMITAZIONE LUNGHEZZA E STILE: Sii estremamente sintetico, chiaro e formale. Le tue risposte non devono MAI superare i 3 paragrafi. Se la risposta è complessa, utilizza obbligatoriamente elenchi puntati per facilitare la lettura veloce agli operatori.
    
    4. GESTIONE INFORMAZIONI MANCANTI: Se la domanda è pertinente ma l'informazione specifica non è contenuta nei documenti, NON inventare nulla (niente allucinazioni). Dì chiaramente: "Mi dispiace, ma non ho trovato questa informazione nella normativa ufficiale fornita."
    
    5. DOMANDE FUORI CONTESTO (OFF-TOPIC): Se l'utente ti fa una domanda completamente estranea all'argomento dell'Assegno Terzo Figlio (ad esempio ricette, meteo, o altre agevolazioni provinciali), NON rispondere alla domanda e usa ESATTAMENTE e SOLO questa frase:
    ...
    try:
        ...
        st.session_state.chat_engine = client.chats.create(
            ...
            config=types.GenerateContentConfig(
                ...
                temperature=0.0 # Temperatura a 0 per evitare divagazioni creative
            )
        )
        
        # Iniettiamo i documenti in blocco una volta sola
...
# Mostriamo a schermo i messaggi precedenti della chat
...
if domanda_utente := st.chat_input("Scrivi qui la tua domanda sull'Assegno Terzo Figlio..."):
    ...
    with st.chat_message("assistant"):
        with st.spinner("Consultando la normativa e le delibere in memoria..."):
            try:
